...
public partial class ResetPasswordSendEmail : ComponentBase
{
  ...
  private async Task SendEmail(AuthForm<EmailModel>.SubmitCallbackArgs args)
  {
    ...
    await HttpHelper.FetchAsync(async () => await AuthApiSource.SendResetPasswordLinkAsync(_model.Email ?? ""),
      onSuccess: _ =>
      {
        _isSuccess = true;
